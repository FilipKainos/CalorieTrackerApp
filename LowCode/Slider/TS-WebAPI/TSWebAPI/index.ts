...
private deleteButtonOnClickHandler(): void {
    ...
    const lookUpPromise: any = this._context.utils.lookupObjects(lookUpOptions);
    ...
    lookUpPromise.then(
        ...
        (data: ComponentFramework.EntityReference[]) => {
            if (data && data[0]) {
                ...
                const id = data[0].id.guid;
                const entityType = data[0].etn!;
                ...
                this._context.webAPI.deleteRecord(entityType, id).then(
                    function (response: ComponentFramework.LookupValue) {
                        ...
                        const responseEntityType = response.name!;
...
private refreshRecordCountButtonOnClickHandler(): void {
    ...
    const queryString = "?$select=" + TSWebAPI._currencyAttributeName + "&$filter=contains(" + TSWebAPI._requiredAttributeName +
...
private renderCreateExample() {
    ...
    const headerDiv: HTMLDivElement = this.createHTMLDivElement("create_container", true, "Click to create "
